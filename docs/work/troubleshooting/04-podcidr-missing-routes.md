---
title: 04. 190노드 중 3노드 PodCIDR 경로 누락
---
...
!!! warning "가장 중요한 선결 확인"
    아래 분석은 **Native routing 구성을 전제**로 합니다. 실제 적용된 라우팅 모드를 먼저 확정하지 않으면,
    **원격 PodCIDR의 Linux 경로가 없다는 관찰만으로는 장애 원인이라고 결론 낼 수 없습니다.**
...
!!! note
    이 전제가 성립하지 않는 라우팅 모드라면 **원격 PodCIDR 경로가 커널에 없는 것 자체는 정상**일 수 있습니다. 1번 항목에서 `routing-mode` 실제 적용값을 먼저 확정해야 하는 이유입니다.
...
`ip route get` 은 호스트 커널 경로의 증거이지만 **Cilium eBPF 터널 경로 전체를 대변하지 않습니다.** `No route to host` 문자열도 경로 부재 외에 ICMP unreachable · 이웃 탐색 실패 · 거부 경로 등으로 발생할 수 있습니다. 캡슐화를 사용하는 구성이라면 정상 / 문제 노드의 ipcache, 원격 노드 IP 도달성, Hubble drop 이유를 함께 비교합니다.

## 6. 판정 기준

| 상황 | 판정 |
| --- | --- |
| Native가 아닌 모드 + 원격 PodCIDR 경로 없음 | **정상일 수 있다.** 누락 경로를 원인으로 확정하지 않는다 |
